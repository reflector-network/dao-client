...
export default class DaoClient {
    ...
    async setDeposit(params) {
        const tx = await this.client.set_deposit({deposit_params: params}, {fee: this.fee})
    ...
    async unlock(developer, operators) {
        ...
        const tx = await this.client.unlock({developer, operators}, {fee: this.fee})
        processSimulationErrors(tx)
        await tx.signAndSend()
    }
    ...
    async claim(claimant, to, amount) {
        ...
        const tx = await this.client.claim({claimant, to, amount}, {fee: this.fee})
        processSimulationErrors(tx)
        await tx.signAndSend()
    }
    ...
    async createBallot(params) {
        const tx = await this.client.create_ballot({params}, {fee: this.fee})
        processSimulationErrors(tx)
    ...
    async retractBallot(id) {
        const tx = await this.client.retract_ballot({ballot_id: id}, {fee: this.fee})
        processSimulationErrors(tx)
        await tx.signAndSend()
    }
    ...
    async vote(id, accepted, simulateOnly = false) {
        const tx = await this.client.vote({ballot_id: id, accepted}, {fee: this.fee})
        processSimulationErrors(tx)
        if (simulateOnly) {
            return tx
        }
